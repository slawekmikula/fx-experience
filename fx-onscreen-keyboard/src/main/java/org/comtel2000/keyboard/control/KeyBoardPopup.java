...
public class KeyBoardPopup extends Popup implements VkProperties {
     ...
     *
     * @param scene     {@link Scene} to connect with the keyboard
     * @param doNotOpen on hidden keyboard do nothing and on showing keyboard
    ...
    /**
     * Set the focus listener as user agent stylesheet for the whole application
     * with main theme Application.STYLESHEET_MODENA.
     *
    ...
    private void rescale(final TextInputControl textNode) {
        ...
        if (keyboard.isStickBottom() && keyboard.getActiveType() != KeyboardType.NUMERIC) {
            ...
            if (keyboard.getActiveType() == KeyboardType.NUMERIC) {
                keyboard.setScale(2.0);
            } else {
                keyboard.setScale(1.0);
            }
